...
@Lazy
@Component
@Command(name = "synchronize", mixinStandardHelpOptions = true, version = "0.1", description = "Prints the checksum (MD5 by default) of a file to STDOUT.")
class SynchronizeCommand implements Runnable {
	...
	public void run() {
		...
		if (directoryToSynchronize.exists()) {
			Collection<File> allFiles = FileUtils.listFiles(directoryToSynchronize,
	...
	private List<PartInfo> generatePartsInfo(File directoryWithSplittedFiles, File sourceFile,
			UserKeys keys) {
		log(stopWatch.formatTime() + " - Signing");
		List<PartInfo> parts = new ArrayList<>();
		String fileId = directoryToSynchronize.toURI().relativize(sourceFile.toURI()).getPath();
